...
const initTime = "05:00"; // 맨처음 화면에 보여줄 값
...
memberEmail.addEventListener("input", (e) => {
  ...
  if (!regExp.test(inputEmail)) {
    ...
    emailMessage.classList.remove('confirm'); // 초록색 글씨 제거 (이전에 컨펌이었을 수도 있음)
    checkObj.memberEmail = false; // 유효하지 않은 이메일임을 기록 (이전에 유효했을 수도 있음)
    return;
  }

  // 5) 유효한 이메일 형식인 경우 중복 검사 수행 (화면이 깜빡이면 안됨)
  // 비동기 요청 (ajax)
...
sendAuthkeyBtn.addEventListener("click", () => {
  ...
  // 새로 발급 받은 인증번호 확인 전까진 checkObj.authKey는 false
  checkObj.authkey = false;
  // 인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText = "";
  ...
  // 클릭 시 타이머 숫자 초기화 (다시 눌러도 05:00 되게)
  min = initMin;
  sec = initSec;
...
memberPw.addEventListener("input", e => {
  ...
  if (inputPw.trim().length === 0) {
    ...
    memberPw.value = "";  // 첫 글자 띄어쓰기 못하게 막기
    return;
  }
  ...
  // 비밀번호 확인에 값이 작성되었을 때 그 때부터 비교 진행
