...
// img 태그에 작성하는 src = 미리보기 이미지를 띄울 URL 주소
let previousImage = profileImg.src;    // 이전 이미지 URL 기록 (바꾸려다 취소하거나 할 때 기억하려고)
                                       // 초기 상태의 이미지 URL 저장

// input (type=file) 태그가 작성할 값 = 서버에 실제로 제출되는 File 객체
let previousFile = null;    // 이전에 선택된 파일 객체를 저장 (바꾸려다 취소하거나 할 때 기억용)

// 이미지 선택 시 미리보기 및 파일 크기 검사
imageInput.addEventListener("change", () => {
    // changer 이벤트 : 기존에 있던 값과 달라지면 발생되는 이벤트
    ...
    if(file) {  // 파일이 선택이 된 경우
        ...
        if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우 (정상인 경우)
            ...
            previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장(다음에 잘못 바뀔 일에 대비)
            previousFile = file;         // 현재 선택된 파일 객체를 이전 파일로 저장 (다음에 잘못 바뀔 일에 대비)
