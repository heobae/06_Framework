...
public class MyPageServiceImpl implements MyPageService {
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// - 현재 로그인 한 회원의 암호화 된 비밀번호를 DB에서 조회 (matches 이용)
		String originPw = mapper.selectPw(memberNo);
		
		// 입력 받은 현재 비밀번호와(평문)
		// DB에서 조회한 비밀번호(암호화)를 비교
		// -> bcrypt.matches(평문, 암호화비번) 사용 (반환하는 값은 boolean타입)
		...
		// DB에 업데이트
		// SQL에 전달해야 하는 데이터 2개 (암호화한 새 비번인 encPw, 회원번호 memberNo)
		// -> mapper에 전달할 수 있는 전달인자는 단 1개!
		// -> 묶어서 전달 (paramMap 재활용)
		
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo + ""); // 1 + "" => 문자열로 변환
	...
	public int secession(String memberPw, int memberNo) {
		...
		// 현재 로그인 한 회원의 암호화 된 비밀번호 DB 조회
		String originPw = mapper.selectPw(memberNo);
		
		// 현재 비번과 입력한 비번이 다를 경우
	...
	public String fileUpload1(MultipartFile uploadFile) throws Exception {
		...
		// 	 원하는 경로에 실제로 전송 (서버의 어떤 경로 폴더에 저장할지 지정)
