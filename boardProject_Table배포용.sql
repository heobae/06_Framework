...
VALUES(SEQ_MEMBER_NO.NEXTVAL, 
			 'admin1@kh.or.kr',
			 '$2a$10$l7j3exfWCwgOV3ocuN0giuyMTasgYBOQv.eM.BFVv9PWfWaIcpxvG',
			 '관리자',
			 '01012341234',
			 NULL,
			 NULL,
...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$l7j3exfWCwgOV3ocuN0giuyMTasgYBOQv.eM.BFVv9PWfWaIcpxvG'
WHERE MEMBER_NO = 1;
...
-- 게시글 번호 / 제목[댓글개수] / 작성자 닉네임 / 작성일 / 조회수 / 좋아요 가수
...
-- DB 이미지 파일명 목록 조회 (게시판/프로필) (05/12)
...
SELECT CHATTING_ROOM_NO
	,(SELECT MESSAGE_CONTENT FROM 
		...
		ORDER BY MESSAGE_NO DESC) -- 최근메시지를 띄움
		WHERE ROWNUM = 1) LAST_MESSAGE -- 한 줄을 띄움 (최근메시지 한 줄을 카톡처럼)
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인 한 사람이 오픈멤버일 때는 참가자가 타겟번호)
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		) TARGET_NICKNAME	-- 위에랑 같은데 닉네임임
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
		(SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.PARTICIPANT),
		(SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.OPEN_MEMBER)
		) TARGET_PROFILE -- 타겟의 사진 가져옴
	,(SELECT COUNT(*) FROM MESSAGE M 
	WHERE M.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO 
	AND READ_FL = 'N' AND SENDER_NO != 1) NOT_READ_COUNT -- 내가 보낸게 아닌걸 읽지 않은 것만 세겠다. 카톡에서 내가 보낸거 1안뜸ㅋㅋ
	,(SELECT MAX(MESSAGE_NO) SEND_TIME 
	FROM MESSAGE M 
	WHERE R.CHATTING_ROOM_NO  = M.CHATTING_ROOM_NO) MAX_MESSAGE_NO -- 채팅방의 가장 최근 메시지 번호 가져옴
FROM CHATTING_ROOM R
WHERE OPEN_MEMBER = 1
OR PARTICIPANT = 1
ORDER BY MAX_MESSAGE_NO DESC NULLS LAST; -- 최근에서 내림차순으로
...
JOIN "MEMBER" M ON B.MEMBER_NO = M.MEMBER_NO
GROUP BY BT.BOARD_NAME, B.BOARD_NO, B.BOARD_TITLE, M.MEMBER_NICKNAME -- 그룹함수에 해당하지 않는 애들 넣기
