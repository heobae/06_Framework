// console.log("ajax-main.js loaded."); (연결 잘 됐나 실험용)
...
function getTotalCount() {  // 함수 정의
  ...
  // fetch() API 코드 작성
  fetch("/ajax/totalCount") // 서버로 "/ajax/totalCount" 로 GET 요청 보내겠다.
  ...
  .then(todoList => {
    ...
    for(let todo of todoList) { // 향상된 for문
      ...
      for(let key of arr) {
        ...
        // 제목이 아닌 경우
        td.innerText = todo[key]; // todo['todoNo'] -> 1
        tr.append(td);  // tr의 마지막요소 현재 td 추가
      }
